# ================================================================
# MODELO SARIMAX SEMANAL - PAPA SUPER CHOLA
# ================================================================
...
# ================================================================
# 6. Base semanal utilizada por SARIMAX
# ================================================================
...
base_semanal_sarimax <- precio_modelado %>%
...
filas_modelables <- seq.int(
  ventana_maxima + 1L,
  nrow(base_semanal_sarimax)
)

full_data_sarimax <- base_semanal_sarimax %>%
  dplyr::slice(filas_modelables)

train_data <- full_data_sarimax %>%
  dplyr::filter(Conjunto == "Entrenamiento") %>%
  dplyr::arrange(Fecha)

test_data <- full_data_sarimax %>%
...
# ================================================================
# 8. Rolling forecast SARIMAX de un paso
# ================================================================

# Reestimar SARIMAX en cada semana de prueba.
rolling_forecast_sarimax <- function(
    train_df,
    test_df,
    xreg_cols
) {
...
resultado_rolling <- rolling_forecast_sarimax(
  train_df = train_data,
  test_df = test_data,
  xreg_cols = variables_sarimax
)

pred_sarimax <- resultado_rolling$predicciones
...
metricas_documento <- data.frame(
  ...
  Valor_Documento = c(1.61, 2.20, 7.98, 8.05, 0.26)
)

# Comparar al mismo nivel de redondeo del documento.
verificacion_documento <- metricas_sarimax %>%
...
stopifnot(
  ...
  !anyNA(xreg_futuro_df[, variables_sarimax])
)

# Preparar todo el historial disponible para el modelo final.
base_final_sarimax <- precio_modelado %>%
  dplyr::transmute(
    Fecha,
  ...
  dplyr::filter(
    ...
    dplyr::if_all(
      dplyr::all_of(variables_sarimax),
      is.finite
    )
  )

stopifnot(
  nrow(base_final_sarimax) == 231L,
  max(base_final_sarimax$Fecha) == as.Date("2026-06-15")
)

y_final_sarimax <- stats::ts(
  base_final_sarimax$Precio_Modelo,
  frequency = 52
)

xreg_final_sarimax <- as.matrix(
  base_final_sarimax[, variables_sarimax, drop = FALSE]
)

xreg_futuro <- as.matrix(
  xreg_futuro_df[, variables_sarimax, drop = FALSE]
)

storage.mode(xreg_final_sarimax) <- "double"
storage.mode(xreg_futuro) <- "double"

# Reajustar SARIMAX con toda la información disponible.
fit_sarimax_final <- forecast::auto.arima(
  y_final_sarimax,
  xreg = xreg_final_sarimax,
  seasonal = TRUE,
  stepwise = TRUE,
  approximation = FALSE
)
...
fc_sarimax_futuro <- forecast::forecast(
  fit_sarimax_final,
  h = horizonte_futuro,
  xreg = xreg_futuro,
  level = c(80, 95)
...
orden_sarimax_final <- forecast::arimaorder(fit_sarimax_final)

pronostico_futuro <- xreg_futuro_df %>%
  dplyr::mutate(
    Semana_Pronosticada = dplyr::row_number(),
    Precio_Pronosticado = as.numeric(fc_sarimax_futuro$mean),
    Limite_Inferior_80 = as.numeric(
      fc_sarimax_futuro$lower[, 1]
    ),
    Limite_Superior_80 = as.numeric(
      fc_sarimax_futuro$upper[, 1]
    ),
    Limite_Inferior_95 = as.numeric(
      fc_sarimax_futuro$lower[, 2]
    ),
    Limite_Superior_95 = as.numeric(
      fc_sarimax_futuro$upper[, 2]
    ),
    Orden_SARIMAX = paste0(
      "SARIMAX(",
      unname(orden_sarimax_final["p"]), ",",
      unname(orden_sarimax_final["d"]), ",",
      unname(orden_sarimax_final["q"]),
      ")(",
      unname(orden_sarimax_final["P"]), ",",
      unname(orden_sarimax_final["D"]), ",",
      unname(orden_sarimax_final["Q"]),
      ")[",
      unname(orden_sarimax_final["Frequency"]),
      "]"
    )
  ) %>%
  dplyr::select(
...
controles_datos <- data.frame(
  Indicador = c(
    ...
    "Semanas observadas",
    "Semanas modelables",
    "Semanas de entrenamiento SARIMAX",
    "Semanas de prueba",
    "Semanas evaluadas",
  ...
  Valor = c(
    ...
    as.character(nrow(full_data_sarimax)),
    ...
    paste(variables_sarimax, collapse = ", "),
    as.character(nrow(base_final_sarimax)),
...
detalle_variables_sarimax <- data.frame(
  Variable = variables_sarimax,
  Descripcion = c(
...
# Exportar un Excel exclusivo de SARIMAX.
writexl::write_xlsx(
  list(
    Metricas_Test = metricas_sarimax,
    ...
    Variables_SARIMAX = detalle_variables_sarimax,
    Faltantes_Clima = faltantes_clima_antes,
    Controles_Datos = controles_datos,
    Versiones = versiones_paquetes,
    Base_Modelo_SARIMAX = full_data_sarimax,
    Base_Semanal = base_semanal_sarimax
  ),
  path = "Resultados_SARIMAX_Semanal.xlsx"
)

# Guardar el gráfico de prueba.
ggplot2::ggsave(
  filename = "sarimax_s_test.pdf",
  plot = g_test,
  width = 7,
  height = 4.8,
  units = "in"
...
ggplot2::ggsave(
  filename = "Grafico_sarimax_s_hist_y_pred.pdf",
...
ggplot2::ggsave(
  filename = "Grafico_sarimax_s_solo_pred.pdf",
