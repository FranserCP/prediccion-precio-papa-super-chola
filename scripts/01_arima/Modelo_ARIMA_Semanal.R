# ================================================================
# MODELO ARIMA SEMANAL - PAPA SUPER CHOLA
# ================================================================
...
# ================================================================
# 5. Base utilizada por ARIMA
# ================================================================

# ARIMA usa el objetivo observado, igual que en el Rmd.
...
# ================================================================
# 7. Rolling forecast ARIMA de un paso
# ================================================================

# Reestimar ARIMA en cada semana de prueba.
rolling_forecast_arima <- function(train_df, test_df) {
  history_df <- train_df %>%
...
resultado_rolling <- rolling_forecast_arima(
  train_df = train_data,
  test_df = test_data
)

pred_arima <- resultado_rolling$predicciones
...
metricas_documento <- data.frame(
  ...
  Valor_Documento = c(1.59, 2.21, 7.90, 7.97, 0.25)
)

# Comparar al mismo nivel de redondeo del documento.
verificacion_documento <- metricas_arima %>%
...
serie_final_arima <- precio_modelado %>%
  dplyr::arrange(Fecha) %>%
  dplyr::transmute(
    Fecha,
...
stopifnot(
  nrow(serie_final_arima) == 232L,
  !anyNA(serie_final_arima$Precio_Modelo),
  max(serie_final_arima$Fecha) == as.Date("2026-06-15")
)

# Reajustar ARIMA con toda la serie modelable disponible.
y_final_arima <- stats::ts(
  serie_final_arima$Precio_Modelo,
  frequency = 52
)

fit_arima_final <- forecast::auto.arima(
  y_final_arima,
  seasonal = FALSE,
  stepwise = TRUE,
  approximation = FALSE
...
fc_arima_futuro <- forecast::forecast(
  fit_arima_final,
  h = horizonte_futuro,
  level = c(80, 95)
)
...
fechas_futuras <- seq.Date(
  from = max(serie_final_arima$Fecha) + 7,
  by = "week",
...
pronostico_futuro <- data.frame(
  ...
  Precio_Pronosticado = as.numeric(fc_arima_futuro$mean),
  Limite_Inferior_80 = as.numeric(fc_arima_futuro$lower[, 1]),
  Limite_Superior_80 = as.numeric(fc_arima_futuro$upper[, 1]),
  Limite_Inferior_95 = as.numeric(fc_arima_futuro$lower[, 2]),
  Limite_Superior_95 = as.numeric(fc_arima_futuro$upper[, 2])
)

# Registrar el orden elegido para el modelo final.
orden_arima_final <- forecast::arimaorder(fit_arima_final)

pronostico_futuro$Orden_ARIMA <- paste0(
  "ARIMA(",
  unname(orden_arima_final["p"]), ",",
  unname(orden_arima_final["d"]), ",",
  unname(orden_arima_final["q"]),
  ")"
)
...
controles_datos <- data.frame(
  Indicador = c(
    ...
    "Semanas observadas",
    "Semanas modelables",
    "Semanas de entrenamiento ARIMA",
    "Semanas de prueba",
    "Semanas evaluadas",
  ...
  Valor = c(
    ...
    as.character(nrow(full_data_arima)),
...
# Exportar un Excel exclusivo de ARIMA.
write_xlsx(
  list(
    Metricas_Test = metricas_arima,
    ...
    Base_Modelo_ARIMA = full_data_arima,
    Base_Semanal = precio_modelado
  ),
  path = "Resultados_ARIMA_Semanal.xlsx"
)

# Guardar el gráfico junto al script.
ggplot2::ggsave(
  filename = "arima_s_test.pdf",
  plot = g_test,
  width = 7,
  height = 4.8,
  units = "in"
...
ggplot2::ggsave(
  filename = "Grafico_arima_s_hist_y_pred.pdf",
...
ggplot2::ggsave(
  filename = "Grafico_arima_s_solo_pred.pdf",
