# ================================================================
# MODELO RANDOM FOREST SEMANAL (SOLO PRECIO) - PAPA SUPER CHOLA
# ================================================================
...
# ================================================================
# 5) Ingeniería de variables y base de Random Forest
# ================================================================

# Función única para reconstruir las variables históricas.
# Se usa tanto en la evaluación como en el pronóstico futuro.
crear_features_rf_precio <- function(data_hist) {
  data_hist %>%
    dplyr::arrange(Fecha) %>%
    dplyr::mutate(
...
full_data_rf <- precio_features %>%
  dplyr::slice(filas_modelables)

train_data <- full_data_rf %>%
  dplyr::filter(Conjunto == "Entrenamiento") %>%
  dplyr::arrange(Fecha)

test_data <- full_data_rf %>%
...
stopifnot(
  ...
  nrow(full_data_rf) == 208L,
  nrow(train_data) == 166L,
  nrow(test_data) == 42L,
  sum(test_data$Evaluar_Test) == 41L,
  sum(is.na(test_data$Precio_Objetivo)) == 1L,
  !anyNA(train_data[, variables_rf_precio]),
  !anyNA(test_data[, variables_rf_precio]),
...
# ================================================================
# 7) Grid Search temporal de Random Forest
# ================================================================
...
grid_rf <- expand.grid(
  mtry = c(
    floor(length(variables_rf_precio) / 3),
    floor(length(variables_rf_precio) / 2)
  ),
  nodesize = c(1L, 5L),
...
resultado_tuning_rf <- tune_rf_rolling(
  train_df = train_data,
  features = variables_rf_precio,
  grid = grid_rf,
  ntree = ntree_rf
)

best_rf_precio <- resultado_tuning_rf$best_params
...
hiperparametros_rf <- data.frame(
  ...
  Valor = c(
    ...
    as.character(best_rf_precio$mtry[[1L]]),
    as.character(best_rf_precio$nodesize[[1L]]),
...
# ================================================================
# 8) Rolling forecast Random Forest de un paso
# ================================================================
...
resultado_rolling_rf <- rolling_forecast_rf(
  ...
  test_df = test_data,
  features = variables_rf_precio,
  best_params = best_rf_precio,
  ntree = ntree_rf
)

pred_rf_precio <- resultado_rolling_rf$predicciones
diagnostico_rolling_rf <- resultado_rolling_rf$diagnostico

stopifnot(
  length(pred_rf_precio) == 42L,
  all(is.finite(pred_rf_precio))
)

# ================================================================
# 9) Predicciones y métricas de prueba
# ================================================================
...
metricas_rf <- calc_metrics(
  real = y_real_test,
  pred = pred_rf_precio,
  nombre_modelo = "Random Forest semanal (Solo precio)"
)
...
predicciones_test <- test_data %>%
  dplyr::transmute(
    ...
    Precio_Predicho = pred_rf_precio,
    Error = dplyr::if_else(
      Evaluar_Test,
...
metricas_documento <- data.frame(
  ...
  Valor_Documento = c(1.69, 2.37, 8.37, 8.30, 0.14)
)
...
# ================================================================
# 10) Modelo final, importancia y pronóstico futuro
# ================================================================

datos_modelo_final_rf <- full_data_rf %>%
  dplyr::filter(!is.na(Precio_Objetivo)) %>%
  dplyr::arrange(Fecha)

stopifnot(
  nrow(datos_modelo_final_rf) == 207L,
  !anyNA(datos_modelo_final_rf[, variables_rf_precio]),
...
formula_rf <- stats::as.formula(
  paste(
    "Precio_Objetivo ~",
    paste(variables_rf_precio, collapse = " + ")
  )
)
...
fit_rf_final <- randomForest::randomForest(
  ...
  data = datos_modelo_final_rf[
    ,
    c("Precio_Objetivo", variables_rf_precio)
  ],
  ntree = ntree_rf,
  mtry = as.integer(best_rf_precio$mtry[[1L]]),
  nodesize = as.integer(best_rf_precio$nodesize[[1L]]),
  importance = TRUE
)
...
stopifnot(
  ...
  !anyNA(hist_forecast_rf$Precio_Para_Rezagos),
  max(hist_forecast_rf$Fecha) == as.Date("2026-06-15")
)

# Comprobar que la reconstrucción reproduce la última fila del modelo.
X_comprobacion_rf <- crear_features_rf_precio(
  hist_forecast_rf
) %>%
  dplyr::slice_tail(n = 1L) %>%
  dplyr::select(dplyr::all_of(variables_rf_precio))

X_referencia_rf <- full_data_rf %>%
  dplyr::slice_tail(n = 1L) %>%
  dplyr::select(dplyr::all_of(variables_rf_precio))

stopifnot(
  isTRUE(all.equal(
    as.numeric(X_comprobacion_rf[1L, ]),
    as.numeric(X_referencia_rf[1L, ]),
    tolerance = 1e-10,
...
for (i in seq_len(horizonte_futuro)) {
  ...
  X_new <- crear_features_rf_precio(
    hist_forecast_rf
  ) %>%
    dplyr::slice_tail(n = 1L) %>%
    dplyr::select(dplyr::all_of(variables_rf_precio))
  ...
  stopifnot(
    nrow(X_new) == 1L,
    ncol(X_new) == length(variables_rf_precio),
    !anyNA(X_new),
...
# 11.1. Precio observado frente a predicción en prueba.
g_test <- ggplot2::ggplot(
  ...
  ggplot2::geom_line(
    ggplot2::aes(
      y = Precio_Predicho,
      color = "Predicción Random Forest"
    ),
    linewidth = 0.8
  ) +
  ggplot2::geom_point(
  ...
  ggplot2::scale_color_manual(
    values = c(
      ...
      "Predicción Random Forest" = "#0072B2"
    )
  ) +
...
# 11.3. Histórico y pronóstico futuro.
...
# ================================================================
# 12) Controles y exportación
# ================================================================

variables_exportar <- data.frame(
  Orden = seq_along(variables_rf_precio),
  Variable = variables_rf_precio,
  stringsAsFactors = FALSE
)
...
# Exportar un Excel exclusivo de Random Forest sin clima.
writexl::write_xlsx(
  list(
    ...
    Base_Modelo_RF = full_data_rf,
    Base_Semanal = precio_modelado
  ),
  path = "Resultados_RF_Semanal.xlsx"
)

# Guardar observado frente a predicho.
ggplot2::ggsave(
  filename = "rf_s_test.pdf",
  plot = g_test,
  width = 7,
  height = 4.8,
  units = "in"
...
ggplot2::ggsave(
  filename = "Grafico_rf_s_importancia.pdf",
  plot = g_importancia,
  width = 7,
  height = 4.8,
  units = "in"
...
ggplot2::ggsave(
  filename = "Grafico_rf_s_hist_y_pred.pdf",
...
ggplot2::ggsave(
  filename = "Grafico_rf_s_solo_pred.pdf",
